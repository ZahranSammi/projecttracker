...
@section('content')
    <section class="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
            <span class="chip chip-primary mb-4">Sprint Board</span>
            <h1 class="section-heading">Kanban Board</h1>
            <p class="section-copy mt-3 max-w-2xl">
                The Glacier board view keeps the exported energy of the Stitch concept while remaining fully local to the Laravel frontend.
            </p>
        </div>
                    ...
                    @foreach ($column['cards'] as $card)
                        <a class="kanban-card block" href="{{ route('issues.show', $card['id']) }}">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <span class="chip {{ ($card['priority'] ?? '') === 'Critical' ? 'chip-danger' : 'chip-primary' }}">{{ $card['type'] }}</span>
                                <span class="text-xs uppercase tracking-[0.24em] text-copy-muted">{{ $card['priority'] }}</span>
                            </div>
